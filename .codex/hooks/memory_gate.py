# -*- coding: utf-8 -*-
"""Stop Hook: remind the Agent to make a minimal archive decision."""
...
PITFALL_SIGNALS = (
    "踩坑", "反复失败", "失败恢复", "乱码", "环境冲突", "无法启动",
    "module not found", "traceback", "permission denied", "timed out",
)
LOG_SIGNALS = (
    "部署", "上线", "发布", "数据迁移", "回滚", "灾难恢复", "架构决策",
    "长期决策", "重大重构", "实机验收", "hook", "deploy", "rollback",
)
COMPLETION_SIGNALS = (
...
def evaluate_gate(
    *,
    ...
    tool_summaries = summarize_tool_signals(tool_signals or [])

    missing: list[str] = []
    if workspace_changed and "current" not in memory_changed:
        missing.append("`memory/CURRENT.md` 未反映本轮工作区改动")
    if (pitfall_detected or tool_summaries) and (workspace_changed or completed) and "pitfalls" not in memory_changed:
        detail = "；".join(tool_summaries)
